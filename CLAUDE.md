...
**Caching** is layered in SQLite (`internal/db/schema.sql`, helpers in `services/cache.go`), all TTL-enforced by comparing `fetched_at` at read time: `library_cache` (2h), `amazon_cache` (24h), `book_cache` (full BookEvent per goodreads_id + library set, 2h), `shelf_cache` (whole result stream, 5min). Schema changes go in `schema.sql` — statements must stay idempotent (`IF NOT EXISTS` / `INSERT OR IGNORE`) since migrations re-run on every startup.

**Other endpoints**: `/api/recommendations` (Open Library subject-based recs, `services/recommendations.go`), `/api/libraries` (autocomplete over the seeded `libraries` table), `/api/shorten` + `/s/{token}` (shareable search shortlinks).
